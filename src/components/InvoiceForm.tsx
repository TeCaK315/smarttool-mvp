...
export default function InvoiceForm() {
  ...
  const generateInvoicePDF = () => {
    ...
    tableRows.push(
      ['', '', 'Подытог:', `₽${invoiceData.subtotal.toFixed(2)}`],
    ...
    const sections = [
      ...
      },
      {
        ...
        content: `Номер счета: ${invoiceData.invoiceNumber}\nДата: ${new Date(invoiceData.date).toLocaleDateString('ru-RU')}\nСрок оплаты: ${new Date(invoiceData.dueDate).toLocaleDateString('ru-RU')}`
      },
      {
        heading: 'Позиции',
        table: {
    ...
    generatePDF({
      ...
      subtitle: `для ${invoiceData.client.name}`,
      sections,
      brandColor: '#5a67d8'
      ...
      const response = await fetch('/api/send-email', {
        ...
        body: JSON.stringify({
          ...
          html: `
            ...
            <p>Направляем вам счет на сумму ₽${invoiceData.total.toFixed(2)}.</p>
    ...
    <div className="max-w-4xl mx-auto p-6 bg-gray-800 rounded-lg">
        ...
        <div className="space-y-6">
          <div className="bg-gray-700 p-6 rounded-lg">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-xl font-semibold text-white">Позиции счета</h2>
              <button
                onClick={addItem}
              ...
              {invoiceData.items.map((item, index) => (
                <div key={item.id} className="bg-gray-600 p-4 rounded-lg">
                  <div className="flex justify-between items-start mb-3">
                    <span className="text-sm text-gray-300">Позиция {index + 1}</span>
          ...
          <div className="bg-gray-700 p-6 rounded-lg">
            <h2 className="text-xl font-semibold text-white mb-4">Итоги</h2>
            <div className="space-y-3">
      ...
      <div className="mt-8 flex flex-wrap gap-4 justify-center">
        <button
          onClick={saveInvoice}
          disabled={isSaving}
          className="flex items-center gap-2 px-6 py-3 bg-gray-600 text-white rounded-lg hover:bg-gray-500 transition-colors disabled:opacity-50"
        >
        ...
        <button
          onClick={sendInvoice}
          disabled={isLoading || !invoiceData.client.name || !invoiceData.client.email}
